...
interface SilverDialogueProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function SilverDialogue({ isOpen, onClose }: SilverDialogueProps) {
  useEffect(() => {
    if (!isOpen) return;
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            i can&apos;t wait to wear these pieces to the event tonight.
          </motion.p>
        </div>
      )}
    </AnimatePresence>
